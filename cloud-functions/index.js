...
const LSky_MAX_SIZE = 10 * 1024 * 1024
const LSky_ALLOWED_TYPES = ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/avif', 'image/svg+xml']

const lskyStore = getStore({ name: 'lsky', consistency: 'eventual' })

function lskyExtFromType (mime) {
...
async function lskyGetMetaList () { return await lskyStore.get('meta:all', { type: 'json' }) || [] }
async function lskySaveMetaList (list) { await lskyStore.setJSON('meta:all', list) }

async function lskyAddImage (id, buffer, meta) {
  await lskyStore.setJSON(`img:${id}`, { data: Array.from(new Uint8Array(buffer)), type: meta.type })
  const list = await lskyGetMetaList(); list.unshift(meta); await lskySaveMetaList(list)
}

async function lskyGetImage (id) {
  const img = await lskyStore.get(`img:${id}`, { type: 'json' })
  return img ? { data: new Uint8Array(img.data), type: img.type } : null
}

async function lskyDeleteImage (id) {
  await lskyStore.delete(`img:${id}`)
  const list = await lskyGetMetaList()
  const idx = list.findIndex(m => m.id === id)
  if (idx !== -1) { list.splice(idx, 1); await lskySaveMetaList(list) }
}

async function lskyHandleUpload (request) {
  const ct = request.headers.get('content-type') || ''
  let buffer, fileName, fileType
  if (ct.includes('multipart/form-data')) {
    const form = await request.formData()
    const file = form.get('image') || form.get('file')
    if (!file) return new Response(JSON.stringify({ error: '未提供文件' }), { status: 400, headers: lskyCors() })
    buffer = await file.arrayBuffer(); fileName = file.name; fileType = file.type
  } else if (ct.includes('application/json')) {
    const body = await request.json()
    if (!body.image) return new Response(JSON.stringify({ error: '未提供文件' }), { status: 400, headers: lskyCors() })
    ...
    return new Response(JSON.stringify({ error: '不支持的 Content-Type' }), { status: 400, headers: lskyCors() })
  }
  if (buffer.byteLength > LSky_MAX_SIZE) return new Response(JSON.stringify({ error: `文件过大，最大 10MB` }), { status: 413, headers: lskyCors() })
  if (!LSky_ALLOWED_TYPES.includes(fileType)) return new Response(JSON.stringify({ error: `不支持的文件类型` }), { status: 415, headers: lskyCors() })
  const id = uuidv4().replace(/-/g, ''); const ext = lskyExtFromType(fileType)
  await lskyAddImage(id, buffer, { id, name: fileName, ext, type: fileType, size: buffer.byteLength, created: Date.now(), updated: Date.now() })
  return new Response(JSON.stringify({ success: true, id, url: `/api/lsky/image/${id}${ext}`, name: fileName, size: buffer.byteLength }), { status: 200, headers: { 'Content-Type': 'application/json', ...lskyCors() } })
}

async function lskyHandleProUpload (request, origin) {
  const ct = request.headers.get('content-type') || ''
  if (!ct.includes('multipart/form-data')) return new Response(JSON.stringify({ status: false, message: '仅支持 multipart/form-data' }), { status: 400, headers: { 'Content-Type': 'application/json', ...lskyCors() } })
  const form = await request.formData(); const file = form.get('file')
  if (!file) return new Response(JSON.stringify({ status: false, message: '未提供文件' }), { status: 400, headers: { 'Content-Type': 'application/json', ...lskyCors() } })
  const buffer = await file.arrayBuffer(); const fileType = file.type
  if (buffer.byteLength > LSky_MAX_SIZE) return new Response(JSON.stringify({ status: false, message: '文件过大' }), { status: 413, headers: { 'Content-Type': 'application/json', ...lskyCors() } })
  if (!LSky_ALLOWED_TYPES.includes(fileType)) return new Response(JSON.stringify({ status: false, message: '不支持的文件类型' }), { status: 415, headers: { 'Content-Type': 'application/json', ...lskyCors() } })
  const id = uuidv4().replace(/-/g, ''); const ext = lskyExtFromType(fileType)
  await lskyAddImage(id, buffer, { id, name: file.name, ext, type: fileType, size: buffer.byteLength, created: Date.now(), updated: Date.now() })
  return new Response(JSON.stringify({ status: true, data: { links: { url: `${origin}/api/lsky/image/${id}${ext}` } } }), { status: 200, headers: { 'Content-Type': 'application/json', ...lskyCors() } })
}

async function lskyHandleServe (url) {
  const id = url.pathname.replace('/api/lsky/image/', '').split('.')[0]
  if (!id) return new Response('Not Found', { status: 404, headers: lskyCors() })
  const img = await lskyGetImage(id)
  if (!img) return new Response('Not Found', { status: 404, headers: lskyCors() })
  return new Response(img.data, { status: 200, headers: { 'Content-Type': img.type, 'Cache-Control': 'public, max-age=31536000, immutable', ...lskyCors() } })
}

async function lskyHandleList () {
  const list = await lskyGetMetaList()
  return new Response(JSON.stringify({ success: true, total: list.length, images: list }), { status: 200, headers: { 'Content-Type': 'application/json', ...lskyCors() } })
}

async function lskyHandleDelete (url) {
  const id = url.pathname.replace('/api/lsky/delete/', '').split('.')[0]
  if (!id) return new Response(JSON.stringify({ error: 'Missing id' }), { status: 400, headers: lskyCors() })
  await lskyDeleteImage(id)
  return new Response(JSON.stringify({ success: true }), { status: 200, headers: { 'Content-Type': 'application/json', ...lskyCors() } })
}

async function lskyHandleFrontend (url) {
...
async function handleLskyRequest (context) {
  ...
  try {
    ...
    if (path === '/api/lsky' || path === '/api/lsky/') return lskyHandleFrontend(url)
    if (path.startsWith('/api/lsky/upload')) {
      if (request.method !== 'POST') return new Response('Method Not Allowed', { status: 405, headers: lskyCors() })
      return lskyHandleUpload(request)
    }
    if (path.startsWith('/api/lsky/image/')) {
      if (request.method !== 'GET') return new Response('Method Not Allowed', { status: 405, headers: lskyCors() })
      return lskyHandleServe(url)
    }
    if (path === '/api/lsky/list' || path === '/api/lsky/list/') {
      if (request.method !== 'GET') return new Response('Method Not Allowed', { status: 405, headers: lskyCors() })
      return lskyHandleList()
    }
    if (path.startsWith('/api/lsky/delete/')) {
      if (request.method !== 'DELETE') return new Response('Method Not Allowed', { status: 405, headers: lskyCors() })
      return lskyHandleDelete(url)
    }
    return new Response('Not Found', { status: 404, headers: lskyCors() })
  } catch (e) {
    return new Response(JSON.stringify({ error: e.message }), { status: 500, headers: { 'Content-Type': 'application/json', ...lskyCors() } })
  }
}
